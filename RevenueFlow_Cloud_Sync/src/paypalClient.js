...
function requirePayPalConfig(config) {
  if (!config.paypal.clientId || !config.paypal.clientSecret) {
    throw new Error("PayPal API credentials are not configured on Cloud Sync.");
  }
}
...
async function diagnosePayPal(config) {
  ...
  const mark = (name, ok, detail = "") => result.checks.push({ name, ok: Boolean(ok), detail });
  if (!result.configured) {
    mark("config", false, "PayPal Client ID or Secret is missing on Cloud Sync.");
    result.ok = false;
    return result;
  }
  mark("config", true, "PayPal credentials are configured server-side.");
  try {
...
module.exports = { diagnosePayPal, fetchPayPalTransactions, normalizeTransaction };
