...
const { parsePayPalCsv } = require("../src/csvImport");

test("imports PayPal CSV rows into RevenueFlow records", () => {
...
"07/01/2026","Sample Buyer","Payment Received","Completed","USD","29.00","-1.20","27.80","buyer@example.com","TXN-1001","Monthly service","INV-1001"`);
  ...
  assert.equal(records[0].provider, "PayPal");
  assert.equal(records[0].source, "paypal-csv");
  ...
  assert.equal(records[0].product, "Monthly service");
