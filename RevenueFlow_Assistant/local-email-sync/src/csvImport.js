...
function normalizePayPalCsvRow(row, index) {
  ...
  const transactionId = first(row, ["transaction_id", "transaction_id_1", "payment_id"]);
  const invoice = first(row, ["invoice_number", "invoice_id", "custom_number", "custom"]);
  const item = first(row, ["item_title", "item_name", "item_id", "subject"]);
  const statusText = first(row, ["status", "transaction_status"]);
  const typeText = first(row, ["type", "transaction_type"]);
  ...
  const date = first(row, ["date", "transaction_initiation_date", "time"]);
  const payerEmail = first(row, ["from_email_address", "payer_email", "customer_email", "email"]);
  const payerName = first(row, ["name", "payer_name", "customer_name"]);
  return {
    provider: "PayPal",
    source: "paypal-csv",
    ...
    amountUsd: gross,
    usd: /usd/i.test(first(row, ["currency", "currency_code"])) ? gross : "",
    currency: first(row, ["currency", "currency_code"]) || "USD",
    ...
    customFields: [
      ...
      { name: "PayPal fee", value: first(row, ["fee"]) },
      { name: "PayPal net", value: first(row, ["net"]) }
...
module.exports = { parseCsv, parsePayPalCsv };
